...
namespace ContraForce.Samples.CwInbound.ContraForce;
...
public sealed class ContraForceClient
{
    ...
    public ContraForceClient(HttpClient http, IOptions<ContraForceOptions> options, ILogger<ContraForceClient> logger)
    {
    ...
    public async Task AddIncidentCommentAsync(string incidentId, string source, string content, CancellationToken cancellationToken)
    {
        var path = $"workspaces/{_options.WorkspaceId}/incidents/{Uri.EscapeDataString(incidentId)}/comments";
    ...
    public async Task CloseIncidentAsync(string incidentId, string source, string? closingComment, CancellationToken cancellationToken)
    {
        var path = $"workspaces/{_options.WorkspaceId}/incidents/{Uri.EscapeDataString(incidentId)}/status";
    ...
    private async Task EnsureSuccessAsync(HttpResponseMessage response, string path, CancellationToken cancellationToken)
    {
        ...
        _logger.LogError(
            ...
            path, (int)response.StatusCode, response.ReasonPhrase, body);
